...
impl PpmFile {
    ...
    fn finish_file(file: &mut File) -> WriteResult {
        writeln!(file, "")
    }
...
mod tests {
    ...
    fn write_ppm_header() {
        ...
        PpmFile::write(&mut output, &canvas).expect("Unable to write the file");
    ...
    fn write_pixel_data_splitting_long_lines() {
        ...
        PpmFile::write(&mut output, &canvas).unwrap();
